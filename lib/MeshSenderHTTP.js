var MeshSenderHTTP = function(author, title, desiredSlot, serverBaseUrl) {
	this.info = {
		"author" 	: author,
		"title"		: title,
		"platform" 	: "THREEJS"
...
MeshSenderHTTP.prototype._getSlots = function() {
	...
	xhr.open("GET", this.serverBaseUrl + "/mesh", false);
...
MeshSenderHTTP.prototype._registerSlot = function() {
	...
	var self = this;
	var xhr = new XMLHttpRequest();
	xhr.open("POST", self.serverBaseUrl + "/mesh/register", true);
...
MeshSenderHTTP.prototype._sendFrame = function(buffer) {
	...
	xhr.open("POST", self.serverBaseUrl + "/mesh/" + this.activeSlot.index + "/frame", true);
...
MeshSenderHTTP.prototype._unregister = function(index, key) {
	...
	xhr.open("POST", self.serverBaseUrl + "/mesh/" + self.activeSlot.index + "/stop", true);
